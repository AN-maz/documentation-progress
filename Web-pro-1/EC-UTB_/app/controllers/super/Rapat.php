...
class Rapat extends Controller
{
    ...
    public function index()
    {
        ...
        $this->view('dashboard_/admin/super/rapat/index', $data);
    ...
    public function detail($id)
    {
        ...
        if (!$agenda || $agenda['kategori'] != 'rapat_internal') {
            header('Location: ' . BASEURL . '/super/rapat');
            exit;
        }
        ...
        $this->view('templates/adminLayout/super/sidebar', $data);
        $this->view('templates/adminLayout/super/mobileNav', $data);
        $this->view('dashboard_/admin/super/rapat/detail', $data);
        $this->view('templates/FooterDash');
    }

    // --- FUNGSI TAMBAH RAPAT (YANG HILANG) ---
    public function add()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_FILES['file_materi']) && $_FILES['file_materi']['error'] === UPLOAD_ERR_OK) {
                ...
                if ($upload['success']) {
                    ...
                    Flasher::setFlash($upload['message'], 'Gagal Upload', 'red');
                    header('Location: ' . BASEURL . '/super/rapat/detail/' . $id_agenda);
                    exit;
                }
            }
    ...
    public function absen_manual()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            header('Location: ' . BASEURL . '/super/rapat/detail/' . $data['id_agenda']);
            exit;
        }
    }
