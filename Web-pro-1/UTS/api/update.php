<?php
header("Access-Control-Allow-Origin: *");
header("Content-Type: application/json; charset=UTF-8");
header("Access-Control-Allow-Methods: POST");

require_once 'config.php';

$database = new Database();
$db = $database->getConnection();

if (
    !empty($_POST['id']) &&
    !empty($_POST['title']) && 
    !empty($_POST['description']) && 
    !empty($_POST['category'])
) {

    $image_update_query = ""; 
    $new_image_name = "";     
    
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        $target_dir = "../imgs/";
        
        if (!is_dir($target_dir)) mkdir($target_dir, 0777, true);

        $file_extension = pathinfo($_FILES["image"]["name"], PATHINFO_EXTENSION);
        $unique_name = time() . '_' . uniqid() . '.' . $file_extension;
        $target_file = $target_dir . $unique_name;

        $allowed_types = ['jpg', 'jpeg', 'png', 'gif'];
        if (!in_array(strtolower($file_extension), $allowed_types)) {
            echo json_encode(["success" => false, "message" => "Format file tidak didukung"]);
            exit();
        }

        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            $image_update_query = ", image_url = :image_url";
            $new_image_name = $unique_name;
        } else {
            echo json_encode(["success" => false, "message" => "Gagal upload gambar"]);
            exit();
        }
    }

    $query = "UPDATE company_info 
              SET title = :title, 
                  description = :description, 
                  category = :category 
                  $image_update_query 
              WHERE id = :id";
    
    $stmt = $db->prepare($query);

    $title = htmlspecialchars(strip_tags($_POST['title']));
    $desc = htmlspecialchars(strip_tags($_POST['description']));
    $cat = htmlspecialchars(strip_tags($_POST['category']));
    $id = htmlspecialchars(strip_tags($_POST['id']));

    $stmt->bindParam(':title', $title);
    $stmt->bindParam(':description', $desc);
    $stmt->bindParam(':category', $cat);
    $stmt->bindParam(':id', $id);

    if (!empty($new_image_name)) {
        $stmt->bindParam(':image_url', $new_image_name);
    }

    if ($stmt->execute()) {
        echo json_encode(["success" => true, "message" => "Data berhasil diperbarui"]);
    } else {
        $error = $stmt->errorInfo();
        echo json_encode(["success" => false, "message" => "Gagal update: " . $error[2]]);
    }

} else {
    // Pesan error baru
    echo json_encode(["success" => false, "message" => "Data ID, Title, Desc, atau Category kosong"]);
}
?>