...
export default function CategoryManagementPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      if (editTarget) {
        await adminService.updateCategory(editTarget.id, categoryName);
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className={`text-sm mt-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
            Tambah, ubah, atau hapus kategori untuk mengelompokkan materi.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className={`p-12 text-center rounded-2xl border ${
          ...
          <p className="font-semibold text-base">Kategori tidak ditemukan</p>
        </div>
      ) : (
            ...
            <tbody className={`divide-y ${isDark ? 'divide-slate-800' : 'divide-slate-100'}`}>
              {filteredCategories.map((cat, idx) => (
                <tr key={cat.id} className={`transition ${isDark ? 'hover:bg-slate-800/40' : 'hover:bg-slate-50'}`}>
                  <td className="py-3.5 px-4 text-xs text-slate-400">{idx + 1}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className={`w-full max-w-md p-6 rounded-2xl border shadow-xl ${
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold mb-1.5 text-slate-400">Nama Kategori</label>
                <input
                  type="text"
                  required
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 rounded-xl text-xs font-semibold bg-indigo-600 text-white hover:bg-indigo-500 transition disabled:opacity-50"
                >
